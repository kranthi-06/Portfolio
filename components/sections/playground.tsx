...
export function PlaygroundSection() {
  ...
  const domains = skillCategories.length > 0 ? skillCategories.map((c, i) => ({
    id: c.title.toLowerCase().replace(/\s+/g, '-'),
    label: c.title,
    icon: i === 0 ? "palette" : i === 1 ? "server" : i === 2 ? "brain" : "code",
    color: c.color || "var(--gradient-1)",
    tools: c.skills.map(s => s.name)
  })) : playground.domains;

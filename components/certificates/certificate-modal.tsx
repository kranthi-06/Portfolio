          ...
          <motion.div className="relative z-10 flex h-[min(850px,92vh)] w-full max-w-6xl flex-col overflow-hidden rounded-[28px]" style={{ background: "var(--bg-elevated)", border: "1px solid var(--glass-border)", boxShadow: "0 32px 100px rgba(0,0,0,.4)" }} initial={{ opacity: 0, scale: 0.97, y: 18 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.97, y: 18 }} transition={{ duration: 0.28, ease: [0.16, 1, 0.3, 1] }}>
            <header className="flex items-center justify-between gap-3 border-b px-4 py-3 sm:px-6" style={{ borderColor: "var(--line)" }}>
              ...
              <div className="flex items-center gap-1">
                {certificate.type === "image" && <><button type="button" aria-label="Zoom out" onClick={() => setZoom((value) => Math.max(1, value - 0.25))} className="certificate-icon-button"><ZoomOut size={17} /></button><button type="button" aria-label="Zoom in" onClick={() => setZoom((value) => Math.min(2.5, value + 0.25))} className="certificate-icon-button"><ZoomIn size={17} /></button></>}
            ...
            <div className="relative min-h-0 flex-1 overflow-auto p-3 sm:p-6" style={{ background: "var(--bg-subtle)" }}>
              {certificate.type === "pdf" ? <iframe src={certificate.src} title={certificate.title} className="h-full min-h-[440px] w-full rounded-xl bg-white" /> : <div className="flex min-h-full min-w-max items-center justify-center"><Image src={certificate.src} alt={certificate.title} width={1800} height={1300} priority className="h-auto max-h-full w-auto max-w-full rounded-xl transition-transform duration-200" style={{ transform: `scale(${zoom})`, transformOrigin: "center" }} /></div>}
            </div>

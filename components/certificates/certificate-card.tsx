      ...
      <div className="p-5">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="mb-1 text-[10px] font-bold uppercase tracking-[0.13em]" style={{ color: "var(--gradient-1)" }}>{certificate.category}</p>
            <h3 className="line-clamp-2 font-display text-lg font-medium leading-snug tracking-tight" style={{ color: "var(--ink)" }}>{certificate.title}</h3>
          </div>
          <div className="mt-0.5 shrink-0 rounded-xl p-2" style={{ background: "var(--accent-soft)", color: "var(--ink-secondary)" }}>
            <Award size={16} />
          </div>
        </div>
        <div className="flex items-center justify-between gap-3 border-t pt-4" style={{ borderColor: "var(--line)" }}>
          <span className="truncate text-xs" style={{ color: "var(--ink-muted)" }}>{certificate.organisation}</span>
          <button type="button" onClick={onView} className="shrink-0 text-xs font-bold transition-opacity hover:opacity-65" style={{ color: "var(--ink)" }}>

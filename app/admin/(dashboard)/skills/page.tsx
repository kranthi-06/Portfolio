...
export default function SkillsPage() {
  ...
  const [originalEditing, setOriginalEditing] = useState<typeof emptySkill & { id?: string }>(emptySkill);
  ...
  const { clearDraft } = useAutoSave(
    `skill_draft_${editing.id || "new"}`,
    editing,
    showEditor
  );

  function openEdit(skill?: Skill) {
    const data = skill ? { ...emptySkill, ...skill } : emptySkill;
    setEditing(data);
    ...
    setTimeout(() => {
      try {
        const draftKey = `skill_draft_${(data as any).id || "new"}`;
  ...
  return (
    <div>
      <div className="admin-page-header flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button onClick={() => openEdit()} className="admin-btn admin-btn-primary"><Plus size={14} /> Add Skill</button>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState icon={<Cpu size={48} />} title="No skills yet" description="Add your programming languages, frameworks, tools, and more."
          action={<button onClick={() => openEdit()} className="admin-btn admin-btn-primary"><Plus size={14} /> Add Skill</button>} />
      ) : (
                ...
                {catSkills.map(skill => (
                  <div key={skill.id} className="admin-card p-3 flex items-center gap-3 group">
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => openEdit(skill)} className="admin-btn admin-btn-ghost admin-btn-sm admin-btn-icon"><Cpu size={12} /></button>
      ...
      <AdminModal open={showEditor} onClose={() => setShowEditor(false)} title={editing.id ? "Edit Skill" : "Add Skill"} maxWidth="440px"
        ...
        <div className="space-y-4">
          <div className="admin-field"><label className="admin-label">Skill Name *</label><AIAssistantField value={editing.name} onChange={val => setEditing(p => ({ ...p, name: val }))}><input className="admin-input" value={editing.name} onChange={e => setEditing(p => ({ ...p, name: e.target.value }))} placeholder="e.g. Python" /></AIAssistantField></div>
